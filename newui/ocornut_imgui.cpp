...
struct OcornutImguiContext
{
    void render(ImDrawList** const _lists, int _count)
    {
        ...
        for (int32_t ii = 0; ii < _count; ++ii)
        {
            ...
            Ctr::IVertexBuffer* vb = uiRenderer->vertexBuffer(m_decl);
    ...
    void create(const void* _data, uint32_t _size, float _fontSize)
    {
        ...
        if (Ctr::ShaderMgr* shaderMgr = device->shaderMgr())
        {
    ...
    void beginFrame(Ctr::InputState* inputState, int32_t _mx, int32_t _my, uint8_t _button, int _width, int _height, char _inputChar, uint8_t _viewId)
    {
        m_viewId = _viewId;
        ImGuiIO& io = ImGui::GetIO();
    ...
    Ctr::IVertexDeclaration*    m_decl;
    const Ctr::IShader*         m_program;
    Ctr::ITexture*              m_texture;
    Ctr::GpuVariable*           s_tex;
    uint8_t                     m_viewId;
